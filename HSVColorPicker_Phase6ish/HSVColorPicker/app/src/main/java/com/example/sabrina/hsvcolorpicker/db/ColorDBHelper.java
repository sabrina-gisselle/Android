...
public class ColorDBHelper extends SQLiteOpenHelper {
    ...
    public static String[] getSelectionArgs(){
        ...
        //float saturationDelta = HSVColor.saturationDelta == 1 ? 1 : 1f/2*(HSVColor.saturationDelta-1);
        //float valueDelta = HSVColor.valueDelta == 1 ? 1f : 1f/2*(HSVColor.valueDelta-1);

        float leftSaturation = saturation - .5f;
        float rightSaturation = saturation + .5f;
        float leftValue = value - .5f;
        float rightValue = value + .5f;
